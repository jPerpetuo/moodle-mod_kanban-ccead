define("mod_kanban/mutations", ["exports", "core/ajax", "core/notification", "core/str", "core/log"], function (_exports, _ajax, _notification, _str, _log) {
  "use strict";
  ...
  class _default {
    ...
    async deleteCard(stateManager, cardId) {
      await this._sendChange('delete_card', stateManager, {
        cardid: cardId
      });
    }
    ...
    async addCard(stateManager, columnId, afterCard) {
      await this._sendChange('add_card', stateManager, {
        columnid: columnId,
        aftercard: afterCard
      });
    }
    ...
    async moveCard(stateManager, cardId, columnId, afterCard) {
      await this._sendChange('move_card', stateManager, {
        cardid: cardId,
        columnid: columnId,
        aftercard: afterCard
      });
    ...
    async deleteColumn(stateManager, columnId) {
      await this._sendChange('delete_column', stateManager, {
        columnid: columnId
      });
    }
    ...
    async addColumn(stateManager, afterColumn) {
      await this._sendChange('add_column', stateManager, {
        aftercol: afterColumn
      });
    }
    ...
    async moveColumn(stateManager, columnId, afterColumn) {
      await this._sendChange('move_column', stateManager, {
        columnid: columnId,
        aftercol: afterColumn
      });
    }
    ...
    async assignUser(stateManager, cardId, userId = 0) {
      await this._sendChange('assign_user', stateManager, {
        cardid: cardId,
        userid: userId
      });
    }
    ...
    async completeCard(stateManager, cardId) {
      await this._sendChange('set_card_complete', stateManager, {
        cardid: cardId,
        state: 1
      });
    }
    ...
    async unassignUser(stateManager, cardId, userId = 0) {
      await this._sendChange('unassign_user', stateManager, {
        cardid: cardId,
        userid: userId
      });
    }
    ...
    async updateCardTitle(stateManager, cardId, title) {
      await this._sendChange('update_card_title', stateManager, {
        cardid: cardId,
        title: title
      });
    }
    ...
    async lockColumn(stateManager, columnId) {
      await this._sendChange('set_column_locked', stateManager, {
        columnid: columnId,
        state: 1
      });
    }
    ...
    async lockColumns(stateManager) {
      await this._sendChange('set_board_columns_locked', stateManager, {
        state: 1
      });
    }
    ...
    async sendDiscussionMessage(stateManager, cardId, message) {
      await this._sendChange('add_discussion_message', stateManager, {
        cardid: cardId,
        message: message
      });
    }
    ...
    async deleteMessage(stateManager, messageId) {
      await this._sendChange('delete_discussion_message', stateManager, {
        messageid: messageId
      });
    }
    ...
    async pushCard(stateManager, cardId) {
      await this._sendChange('push_card_copy', stateManager, {
        cardid: cardId
      });
    }
    ...
    async _sendChange(method, stateManager, data) {
      ...
      let result = null;
      try {
        result = await _ajax.default.call([request])[0];
      } catch (e) {
        if (!(e instanceof Object)) {
          ...
          _log.default.warn('Sending a change request to the kanban backend failed, probably due to connection loss.');
    ...
    async getUpdates(stateManager) {
      ...
      if (state.board === undefined) {
        ...
        let result = null;
        try {
          result = await _ajax.default.call([{
    ...
    async displayError(data) {
      if (data.message) {
        ...
        await _notification.default.alert((0, _str.get_string)('error'), data.message, (0, _str.get_string)('cancel'));
      }
    }
    ...
    async getDiscussionUpdates(stateManager, cardId) {
      ...
      const result = await _ajax.default.call([{
    ...
    async duplicateCard(stateManager, cardId) {
      await this._sendChange('duplicate_card', stateManager, {
        cardid: cardId
      });
    }
  }
