...
class change_kanban_content extends external_api {
    ...
    /**
     * Update the title of a card on the board.
     *
     * @param int $cmid The course module id of the kanban board.
     * @param int $boardid The id of the kanban board.
     * @param array $data containing 'cardid' and 'title'
     * @return array The updated data formatted as update message.
     * @throws coding_exception
     * @throws invalid_parameter_exception
     * @throws restricted_context_exception
     * @throws moodle_exception
     */
    public static function update_card_title(int $cmid, int $boardid, array $data): array {
        $params = self::validate_parameters(self::update_card_title_parameters(), [
            'cmid' => $cmid,
            'boardid' => $boardid,
            'data' => $data,
        ]);
        ...
        $boardid = $params['boardid'];
        $cardid = $params['data']['cardid'];
        $title = clean_param($params['data']['title'], PARAM_TEXT);
        [$course, $cminfo] = get_course_and_cm_from_cmid($cmid);
        $context = context_module::instance($cmid);
        self::validate_context($context);
        ...
        $boardmanager->update_card($cardid, ['title' => $title]);
    ...
    /**
     * Returns description of method parameters for the update_card_title function.
     *
     * @return external_function_parameters
     */
    public static function update_card_title_parameters(): external_function_parameters {
        return new external_function_parameters([
            'cmid' => new external_value(PARAM_INT, 'course module id', VALUE_REQUIRED),
            'boardid' => new external_value(PARAM_INT, 'board id', VALUE_REQUIRED),
            'data' => new external_single_structure([
                'cardid' => new external_value(PARAM_INT, 'id of the card to update', VALUE_REQUIRED),
                'title' => new external_value(PARAM_TEXT, 'new card title', VALUE_REQUIRED),
            ]),
        ]);
    }

    /**
     * Definition of return values of the update_card_title function.
     *
     * @return external_single_structure
     */
    public static function update_card_title_returns(): external_single_structure {
        return self::default_returns();
    }
}
